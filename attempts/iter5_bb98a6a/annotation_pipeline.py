...
def _one_pass(model, system_prompt, markdown_content):
    """One extraction pass. Robust: any error -> empty lists (degrade, don't crash)."""
    try:
...
# Ensemble passes: (model, system_prompt). Two gpt-5.4 readers (primary +
# easy-to-miss focus) plus a different model family (Claude) for genuine
# extraction diversity. Union maximizes recall; extras are not penalized.
PASSES = [
    (MODEL, SYSTEM_PROMPT),
    (MODEL, SECOND_PASS_PROMPT),
    ("claude-sonnet-4-5", SYSTEM_PROMPT),
]


def predict(markdown_content):
    results = [_one_pass(m, p, markdown_content) for m, p in PASSES]

    # Union variants (dedup case-insensitively, preserving first-seen casing).
    variants, seen_v = [], set()
    for v in [v for vs, _ in results for v in vs]:
    ...
    for s in [s for _, ss in results for s in ss]:
