...
def _gapfill_pass(markdown_content, draft_sentences):
    listing = "\n".join(f"- {s}" for s in draft_sentences)
    user = (
        f"{markdown_content}\n\n=== ALREADY-EXTRACTED ASSOCIATIONS (add only what is "
        f"missing) ===\n{listing}"
    )
    return _one_pass(GAPFILL_SYSTEM, user)
...
def predict(markdown_content):
    ...
    for s in list(s1) + list(s2):
        k = str(s).strip().lower()
        if k and k not in seen0:
            seen0.add(k)
            draft.append(s)
    v3, s3 = _gapfill_pass(markdown_content, draft)

    # Union variants (dedup case-insensitively, preserving first-seen casing).
    variants, seen_v = [], set()
    for v in list(v1) + list(v2) + list(v3):
    ...
    for s in list(s1) + list(s2) + list(s3):
