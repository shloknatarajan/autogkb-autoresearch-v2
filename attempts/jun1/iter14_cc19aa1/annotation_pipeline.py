...
def _build_user_content(markdown_content, regex_variants):
    """Append a deterministic regex-extracted variant inventory to the paper text.

    The regex scan (tools/regex_variants) reliably catches EVERY rsID / star /
    HLA allele token in the text -- including the long allele panels (HLA studies)
    the LM tends to summarize away. Injecting them forces per-variant coverage."""
    if not regex_variants:
        return markdown_content
    inv = ", ".join(sorted(regex_variants))
    return (
        f"{markdown_content}\n\n"
...
def predict(markdown_content):
    # Deterministic variant inventory injected into both LM passes (regex-in-the-loop).
    regex_variants = extract_all_variants(markdown_content)
    user_content = _build_user_content(markdown_content, regex_variants)

    v1, s1 = _one_pass(SYSTEM_PROMPT, user_content)
    v2, s2 = _one_pass(SECOND_PASS_PROMPT, user_content)

    # Union variants -- regex hits first (guaranteed coverage), then LM-found.
    variants, seen_v = [], set()
    for v in list(regex_variants) + list(v1) + list(v2):
