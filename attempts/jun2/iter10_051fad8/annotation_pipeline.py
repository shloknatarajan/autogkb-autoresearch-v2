...
MODEL = "gpt-5.4"
# A second, architecturally different reader. Model diversity tends to surface
# variants/associations the primary model skips (complementary recall), which
# fills otherwise-missing variant keys -- the change that has reliably helped.
...
def _one_pass(system_prompt, markdown_content, model=MODEL):
    ...
    # OpenAI supports JSON mode; for other providers (e.g. Anthropic) we rely on
    # the "Return JSON only" instruction + the balanced-brace extractor, and must
    # supply max_tokens so the (potentially large) object isn't truncated.
...
def predict(markdown_content):
    # Two readers (the proven sweet spot -- a 3rd pass dilutes the judge), but the
    # second reader is a DIFFERENT model for complementary coverage of variants
    # the primary model misses.
    g1 = _one_pass(SYSTEM_PROMPT, markdown_content)
    g2 = _one_pass(SECOND_PASS_PROMPT, markdown_content, model=MODEL2)
