...
# A third reader for association types the first two passes systematically miss.
# In iter7 (no cap) this diluted the judge and hurt; with the per-variant CAP now
# trimming long tails, its extra recall may pay off without the dilution penalty.
THIRD_PASS_PROMPT_SUFFIX = """
...
THIRD-PASS FOCUS: recover associations the previous readers systematically miss.
  ...
  - rsID GENOTYPE-LEVEL drug associations: when an rsID is reported by nucleotide
    genotypes (CC/CT/TT, AA/AG/GG), emit a sentence per genotype group for dose,
    efficacy, risk or toxicity, filed under the rsID.
...
def predict(markdown_content):
    ...
    g3 = _one_pass(SYSTEM_PROMPT + THIRD_PASS_PROMPT_SUFFIX, markdown_content)
