...
# A third reader targeting association types the first two passes tend to miss:
# functional/molecular effects, rsID genotype-level drug associations, and
# metabolizer-status phenotypes. Union with the others is purely additive.
THIRD_PASS_PROMPT = (
    SYSTEM_PROMPT
    + """

THIRD-PASS FOCUS: recover associations the previous readers systematically miss.
  - FUNCTIONAL / MOLECULAR effects: a variant/genotype "is associated with
    increased/decreased activity/function/expression/metabolism of GENE"
    (e.g. "Genotypes CT + TT is associated with decreased activity of DPYD as
    compared to genotype CC"). Emit these for every enzyme-activity finding.
  - rsID GENOTYPE-LEVEL drug associations: when the paper reports an rsID by its
    nucleotide genotypes (CC/CT/TT, AA/AG/GG, etc.), emit a sentence per genotype
    group for dose, efficacy, risk, or toxicity, filed under the rsID.
  - METABOLIZER STATUS: poor / intermediate / normal / rapid / ultrarapid
    metabolizer phenotypes linked to a variant or diplotype.
  - Express the SAME association multiple ways (per-allele and per-genotype) so at
    least one framing matches. Be exhaustive; extra sentences are not penalized."""
)
...
def predict(markdown_content):
    ...
    g3 = _one_pass(THIRD_PASS_PROMPT, markdown_content)
