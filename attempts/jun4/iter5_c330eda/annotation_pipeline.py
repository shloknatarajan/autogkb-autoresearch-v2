...
def predict(markdown_content):
    ...
    # Fill-missing-only cross-filing: PharmGKB files each association under every
    # allele it names (incl. the <GENE>*1 reference). meaning_capture is macro per
    # variant, so a gold allele key the model left EMPTY scores 0. Cross-filing
    # surfaces those keys -- but adding sentences to an already-populated variant
    # dilutes the judge and hurts the many single-sentence variants. So we keep
    # the model's own sentences for any populated key, and only accept cross-filed
    # sentences for keys that were absent or empty.
    ...
    for k, sents in crossed.items():
        if k in populated:
            continue  # don't dilute keys the model already answered
